...
for cc in [c for c in ('gcc','clang') if shutil.which(c)]:
  with tempfile.TemporaryDirectory() as td:
    exe=Path(td)/'t'
    cmd=[cc,'-std=c11','-O2','-Wall','-Wextra','-Werror',f'-I{R}',f'-I{R/"Src"}',f'-I{R/"tools/support/hall_detect_stubs"}',str(R/'tools/tests/host/test_foc_math.c'),str(R/'Src/motor/foc_math.c'),'-lm','-o',str(exe)]
