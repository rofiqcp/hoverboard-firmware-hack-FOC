...
assert re.search(r'#define\s+NB_OF_VAR\s+260u',eeh)
util=(R/'Src/util.c').read_text(); addrs=[int(x) for x in re.search(r'VirtAddVarTab\[NB_OF_VAR\]\s*=\s*\{([^}]*)\}',util,re.S).group(1).split(',')]; assert addrs==list(range(1000,1260))
