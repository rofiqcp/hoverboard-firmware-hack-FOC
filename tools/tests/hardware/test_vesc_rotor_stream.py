...
def main():
    ...
    try:
        for right in (False,True):
            ...
            send(prefix+[COMM_SET_DETECT,MODE_OBSERVER]); ts=[]; pos=[]; end=time.monotonic()+a.seconds
            ...
            diff=wrapped_diff(pos[-1],v.position) if pos and v else 999.0
            good=len(pos)>=int(a.seconds*85) and 85<=hz<=115 and v is not None and abs(diff)<1.0 and v.fault==0
            print(f'{"RIGHT-ID2" if right else "LEFT-ID1"}: n={len(pos)} rate={hz:.2f}Hz avg_period={statistics.mean(periods) if periods else 0:.2f}ms stream={pos[-1] if pos else 0:.5f}deg values={v.position if v else 0:.5f}deg diff={diff:.5f} fault={v.fault if v else -1} RESULT={"PASS" if good else "FAIL"}')
            ok &= good
    finally:
