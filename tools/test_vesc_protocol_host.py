...
for cc in [c for c in ('gcc','clang') if shutil.which(c)]:
  with tempfile.TemporaryDirectory() as td:
    ...
    exe=td/'p'
    cmd=[cc,'-std=c11','-O2','-Wall','-Wextra','-Werror',f'-I{ROOT}',f'-I{ROOT/"Src"}',f'-I{ROOT/"Src/vesc"}',f'-I{td}',str(ROOT/'tools/test_vesc_protocol_host.c'),str(ROOT/'Src/vesc/vesc_protocol.c'),str(ROOT/'Src/vesc/buffer.c'),str(ROOT/'Src/vesc/crc.c'),str(ROOT/'Src/vesc/mcconf_serial.c'),'-lm','-o',str(exe)]
