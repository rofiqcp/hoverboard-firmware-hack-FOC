...
int32_t confgenerator_serialize_mcconf(uint8_t *buffer, const mc_configuration *conf) {
    ...
    buffer[ind++] = (conf->foc_control_sample_mode != FOC_CONTROL_SAMPLE_MODE_V0) ? 1u : 0u;
    buffer[ind++] = (conf->foc_current_sample_mode == FOC_CURRENT_SAMPLE_MODE_HIGH_CURRENT) ? 1u : 0u;
    ...
    buffer[ind++] = conf->foc_offsets_cal_mode ? 1u : 0u;
    ...
    /* GPD fields existed in 6.00 but are not used by this bare-metal FOC port. */
    buffer_append_int16(buffer, 0, &ind);
    buffer_append_int16(buffer, 0, &ind);
    buffer_append_float16(buffer, 0.0f, 10000, &ind);
...
bool confgenerator_deserialize_mcconf(const uint8_t *buffer, mc_configuration *conf) {
    ...
    conf->foc_control_sample_mode = buffer[ind++] ? FOC_CONTROL_SAMPLE_MODE_V0_V7 : FOC_CONTROL_SAMPLE_MODE_V0;
    conf->foc_current_sample_mode = buffer[ind++] ? FOC_CURRENT_SAMPLE_MODE_HIGH_CURRENT : FOC_CURRENT_SAMPLE_MODE_LONGEST_ZERO;
    ...
    conf->foc_offsets_cal_mode = buffer[ind++] ? 1u : 0u;
    ...
    (void)buffer_get_int16(buffer, &ind); (void)buffer_get_int16(buffer, &ind);
    (void)buffer_get_float16(buffer, 10000, &ind); (void)buffer_get_float32_auto(buffer, &ind); (void)buffer_get_float32_auto(buffer, &ind);
