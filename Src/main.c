...
extern volatile uint32_t foc_isr_cycles_last;
...
typedef struct{
  ...
  int16_t  boardTemp;
  uint16_t  cmdLed;
  uint16_t  isrCountLo;   // low 16 bits of foc_isr_count
  uint16_t  isrCountHi;   // high 16 bits of foc_isr_count
  uint16_t  isrCyclesLast; // cycles spent in last ISR
  uint16_t  isrCyclesMax;  // max cycles spent in any ISR
  uint16_t  checksum;
} SerialFeedback;
...
int main(void) {
  ...
  // Enable Cortex-M3 DWT cycle counter for real ISR cycle budget measurement.
      ...
      if (main_loop_counter % 2 == 0) {    // Send data periodically every 10 ms
        ...
        Feedback.boardTemp	    = (int16_t)board_temp_deg_c;
          ...
          if(__HAL_DMA_GET_COUNTER(huart2.hdmatx) == 0) {
            ...
            Feedback.isrCountLo  = (uint16_t)(foc_isr_count & 0xFFFFU);
            Feedback.isrCountHi  = (uint16_t)(foc_isr_count >> 16);
            Feedback.isrCyclesLast = (uint16_t)foc_isr_cycles_last;
            Feedback.isrCyclesMax  = (uint16_t)foc_isr_cycles_max;
            Feedback.checksum   = (uint16_t)(Feedback.start ^ Feedback.cmd1 ^ Feedback.cmd2 ^ Feedback.speedR_meas ^ Feedback.speedL_meas 
                                           ^ Feedback.wheelR_cnt ^ Feedback.wheelL_cnt ^ Feedback.batVoltage ^ Feedback.boardTemp ^ Feedback.cmdLed
                                           ^ Feedback.isrCountLo ^ Feedback.isrCountHi
                                           ^ Feedback.isrCyclesLast ^ Feedback.isrCyclesMax);
